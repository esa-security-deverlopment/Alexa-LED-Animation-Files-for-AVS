...
class MyScene(Scene):
	def setup(self):
		...
		self.instructions = LabelNode(
			'Draw with one finger.', ('HelveticaNeue-Light', 24),
	...
	def touch_moved(self, touch):
		...
		label.run_action(
			A.sequence(
				...
				A.wait(3), A.scale_to(0, 0.25), A.remove()
			)
		)
		...
		for i in range(5):
			...
			p.run_action(
				A.sequence(
					A.group(A.scale_to(0), A.move_by(dx, dy, 0.5, TIMING_EASE_OUT_2)),
					A.remove()
				)
			)
			self.add_child(p)
